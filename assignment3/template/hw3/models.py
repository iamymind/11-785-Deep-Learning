...
class LSTMModelV2(nn.Module):
    ...
    def __init__(self, word_count, embedding_dim, hidden_dim, dropout_prob=0.5):
        ...
        self.dropout   = nn.Dropout(dropout_prob)
        self.embedding = nn.Embedding(num_embeddings = word_count, embedding_dim=embedding_dim)
        ...
        self.projection = nn.Linear(in_features=embedding_dim, out_features=word_count)
        #weight tieing
        self.embedding.weight = self.projection.weight
        self.init_embedding()
    ...
    def forward(self, x, hidden, forward=0, stochastic=False):
        ...
        if forward > 0:
            ...
            for i in range(forward):
                ...
                if stochastic:
                    gumbel = utils.to_variable(utils.sample_gumbel(shape=output.size(), out=.data.new()))
                    output += gumbel
